//#region node_modules/simplelightbox/dist/simple-lightbox.esm.js
/*!
...
var SimpleLightbox = class {
	...
	constructor(elements, options) {
		...
		} else this.elements = typeof elements.length !== "undefined" && elements.length > 0 ? Array.from(elements) : [elements];
		...
		if (this.options.docClose) this.addEventListener(this.domNodes.wrapper, ["click." + this.eventNamespace, "touchstart." + this.eventNamespace], (event) => {
		...
		if (this.options.enableKeyboard) this.addEventListener(document.body, "keyup." + this.eventNamespace, this.throttle((event) => {
			...
			if (this.isOpen) {
				...
				if (!this.isAnimating && ["ArrowLeft", "ArrowRight"].indexOf(event.key) > -1) this.loadImage(event.key === "ArrowRight" ? 1 : -1);
			}
	...
	loadImage(direction) {
		...
		if (this.options.animationSlide) this.slide(this.options.animationSpeed / 1e3, -100 * slideDirection - this.controlCoordinates.swipeDiff + "px");
	...
	adjustImage(direction) {
		...
		tmpImage.addEventListener("load", (event) => {
			...
			if (this.options.captions && captionContainer) if (this.options.captionType === "data") captionText = captionContainer.dataset[this.options.captionsData];
	...
	addEvents() {
		...
		if (this.options.scrollZoom) {
			...
			this.addEventListener(this.domNodes.image, ["mousewheel", "DOMMouseScroll"], (event) => {
				...
				if (delta === void 0) delta = event.detail;
		...
		this.addEventListener(this.domNodes.image, [
			...
			if (this.isTouchDevice && event.type === "touchend") {
				...
				if (this.controlCoordinates.touchCount === 0) {
					if (this.currentImage) this.setZoomData(this.controlCoordinates.initialScale, this.controlCoordinates.targetOffsetX, this.controlCoordinates.targetOffsetY);
					if (this.controlCoordinates.initialScale === 1) {
						this.controlCoordinates.zoomed = false;
						if (this.domNodes.caption.style.display === "none") this.fadeIn(this.domNodes.caption, this.options.fadeSpeed);
					}
			...
			if (this.controlCoordinates.mousedown) {
				...
				else if (this.options.animationSlide) this.slide(this.options.animationSpeed / 1e3, "0px");
	...
	updateHash() {
		...
		else if (this.historyHasChanges) window.location.replace(newURL);
	...
	updateURL() {
		clearTimeout(this.historyUpdateTimeout);
		if (!this.historyHasChanges) this.updateHash();
		...
		for (let element of elements) {
			...
			for (let event of events) {
				...
				if ([
					...
					"mousewheel",
					"DOMMouseScroll"
				].indexOf(event.split(".")[0]) >= 0 && this.isPassiveEventsSupported) if (typeof options === "object") options.passive = true;
	...
	removeEventListener(elements, events) {
		elements = this.wrap(elements);
		events = this.wrap(events);
		for (let element of elements) for (let event of events) if (element.namespaces && element.namespaces[event]) {
	...
	off(events) {
		events = this.wrap(events);
		for (let element of this.elements) for (let event of events) if (typeof element.fullyNamespacedEvents !== "undefined" && event in element.fullyNamespacedEvents) element.removeEventListener(event, element.fullyNamespacedEvents[event]);
		return this;
	}
...
//#endregion
export { SimpleLightbox as default };

//# sourceMappingURL=simplelightbox_dist_simple-lightbox__esm__js.js.map